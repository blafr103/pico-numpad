...
**Status - Rev A.** Design complete, DRC clean, full design review passed. Boards fabricated by PCBWay (ENIG) and hand-assembled. Electrical bring-up is **in progress and incomplete** - the board does not yet enumerate over USB and the 3.3 V rail is under investigation. The C firmware targeting this board has been started but is not yet functional. Treat the design as unvalidated.
...
- 5x4 diode-per-key matrix, one 1N4148 per switch, **cathode to row** (COL2ROW). This is the reverse of the hand-wired build, which is wired anode to row.
...
│   ├── numpad.csv                       BOM export from the schematic
│   ├── part-numbers.txt                 working notes on ordered part numbers
...
│   └── W1129510AS1K1.pdf                PCBWay order document, W1129510AS1K1
├── stencil_gerbers/                     paste and outline set sent for the SMD stencils
├── stencil_gerbers.zip                  the same set, zipped for upload
├── production file W1129510AS1K2/       PCBWay's returned CAM data for W1129510AS1K2
└── production file W1129510AS1K2.zip    the same, as delivered
```

`numpad/lib/marbastlib` is a submodule. After cloning:
...
17 keys plus the encoder push occupy 18 of the 20 cells in a 5x4 grid. The 2u-tall `+` and `Enter` keys each take a single electrical cell, at R1C3 and R4C3, which leaves R2C3 free for the encoder push and R3C3 unused. The 2u-wide `0` key leaves R4C1 unused.
...
Exact GPIO assignments live in the schematic and are the source of truth for firmware. Verified against the KiCad netlist: RGB data on **GP11**, encoder A/B on **GP7/GP8**. The matrix rows (GP2-GP6), columns (GP12-GP15), and I2C (GP0/GP1) carry over unchanged from the hand-wired build. GP11 changes role, it drove the single-colour LED PWM on the hand-wired unit and drives the SK6812 chain here. The encoder is new to this board.
...
The hand-wired numpad in the repository root runs **CircuitPython** on a Pico W. That firmware does **not** port directly to this board - the bare RP2040 boot flow, the encoder, and RGB (vs single-color PWM) all differ, as does the diode orientation. The matrix and I2C pin assignments do carry over.
 
The custom board is designed to run a **C firmware on pico-sdk + TinyUSB**. That rewrite has been started and is not yet functional.

---

## License

The files in this repository are covered by the license terms in the repository root.
 
The KiCad symbols and footprints under `numpad/lib/marbastlib` are a git submodule of [ebastler/marbastlib](https://github.com/ebastler/marbastlib), licensed **CERN-OHL-P-2.0**. Those terms, including its notice requirements, apply to that library and to the footprints used from it.
